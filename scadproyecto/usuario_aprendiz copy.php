...
include('funciones.php');  
        ...
        $miconexion=conectar_bd('', 'bd_proyecto');
            $verificacion=consulta($miconexion,"SELECT  EXISTS(select * from correo_usuario where trim(nombre_correo) like '%{$_POST['correo']}%' and  trim(contraseña) like '%{$_POST['clave']}%') ");
            $row=mysqli_fetch_row($verificacion);

            if ($row[0]=="1" or (isset ($_SESSION['nusuario'])==true ) )  
            {
       
    
    $miconexion=conectar_bd("",  $base_datos); 
        
    $resultad=consulta($miconexion,"select * from correo_usuario where nombre_correo='{$_POST['correo']}' and contraseña='{$_POST['clave']}'");
  ...
  if (($idusuario==2) or (isset ($_SESSION['nusuario'])==true)) {
...
<body id="page-top">
  ...
  <nav class="navbar navbar-expand-lg bg-secondary text-uppercase fixed-top" id="mainNav">
    <div class="container">
        <a class="navbar-brand js-scroll-trigger" href="#page-top">SCAD - Control de Asistencia Digital</a>
                      ...
                      <center><div class="doc">
                          <center><div class="tip1">
                                     <?php
                                        ...
                                        echo"<label class='lgris'><h1>Género</h1></label> <br> ";    
                                        ...
                                        echo"<input type='radio' name='sexo' value='".$sex."' /> Masculino ";
                                        echo"<input type='radio' name='sexo' value='".$sex."' /> Femenino ";
                                        echo"<input type='radio' name='sexo' value='".$sex."' /> Otro ";
            ...
            <div class="datosbasic2">
                <ul class="form-section-closed" style="height: 60px;clear:both;" id="section_175">
                  ...
                  <div id="cid_32" class="form-input-wide">
                    ...
                    <select id="p"class="form-control" name="tipo_p" require/>                        
                                <option class="form-control" value="0"><h1>Tipo de permiso</h1></option>
                                        <?php
                                         $mysqli = new mysqli('127.0.0.1', 'root', '', $base_datos);
                                        $query = $mysqli -> query ("SELECT * FROM $tabla4");
                                        while ($valores = mysqli_fetch_array($query)) {
                                            echo '<option value="'.$valores[$id_tipoper].'">'.$valores[$nombre_per].'</option>';
                                        }  
                                      
                                        ?>  
                            </select>  
                            <select id="p"class="form-control" name="autoriza" require/>                        
                                <option class="form-control" value="0"><h1>QUIEN AUTORIZA</h1></option>
                                        <?php
                                         $mysqli = new mysqli('127.0.0.1', 'root', '', $base_datos);
                                        $query = $mysqli -> query ("SELECT * FROM $tabla3 where trim($idtipousu_datos) like '%1%'");
                                        while ($valores = mysqli_fetch_array($query)) {
                                            echo '<option value="'.$valores[$id_datos].'">'.$valores[$nomb_datos].'   '.$valores[$ape_datos].'</option>';
                                        }  
                            ...
                            <form class="jotform-form" method="post"  name="form_210896882135666" id="210896882135666" accept-charset="utf-8" autocomplete="on">
                                ...
                                <textarea class="form-control" type="input" rows="5"name="nota" cols="50" placeholder="crear nota"></textarea>
                                <button id="guardar_a"class="btn btn-primary" type="submit" name="usuario" value="3" >GUARDAR PERMISO </button>  
...
/*------------------------------------------hora---------------------------------------------*/
        ...
        ?>
            ...
            <button id="guardar_apre"class="btn btn-primary" type="submit" name="usuario" value="3" >ACTUALIZAR DATOS </button>  
...
<?php
                ...
                echo '<script> alert("Error<br>el usuario y contraseña son incorrecto") </script>';
